...
Saves CSVs into ENDTERM/ as <TICKER>_test.csv (so your env_trading can load them with split="test").
...
# If the script accidentally lives inside ENDTERM, treat project root as the parent
if os.path.basename(ROOT_DIR).lower() == "endterm":
...
HARD_CODED_TICKERS = [
    ...
    "NTPC.NS", "ONGC.NS", "TATAMOTORS.NS", "SRF.NS", "HDFCLIFE.NS", "SBILIFE.NS",
    ...
    "ACC.NS", "LICI.NS", "SBIN.NS"  # duplicates are benign and will be skipped on save
]
...
def download_and_save_ticker(ticker: str, start_date: dt.date, end_date: dt.date, out_dir: str):
    ...
    out_path = os.path.join(out_dir, f"{safe_ticker}_test.csv")
...
def main():
    ...
    if not downloaded_files:
        ...
        out = download_and_save_ticker("^NSEI", START_DATE, END_DATE, ENDTERM_DIR)
        if out:
            downloaded_files.append(out)

    print(f"[done] Downloaded {len(downloaded_files)} files into {ENDTERM_DIR}")
